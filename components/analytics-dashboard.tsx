...
import { telemetry, type PerformanceMetrics, type UserBehaviorMetrics, type SystemHealthMetrics } from "@/lib/telemetry"
import {
  Activity,
  BarChart3,
  Clock,
...
export function AnalyticsDashboard() {
  ...
  const getHealthStatus = (
    ...
    if (value <= thresholds.warning) {
      return { status: "warning", color: "text-yellow-400", icon: <AlertTriangle className="w-4 h-4" /> }
    }
    return { status: "critical", color: "text-red-400", icon: <AlertTriangle className="w-4 h-4" /> }
  }
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <IndustrialCard className="p-6 text-center">
                ...
                <div className="text-2xl font-bold text-white mb-1">{userBehaviorMetrics?.promptsGenerated || 0}</div>
              ...
              <IndustrialCard className="p-6 text-center">
                ...
                <div className="text-2xl font-bold text-white mb-1">{userBehaviorMetrics?.optimizationsUsed || 0}</div>
              ...
              <IndustrialCard className="p-6 text-center">
                ...
                <div className="text-2xl font-bold text-white mb-1">{userBehaviorMetrics?.testsExecuted || 0}</div>
              ...
              <IndustrialCard className="p-6 text-center">
                ...
                <div className="text-2xl font-bold text-white mb-1">{userBehaviorMetrics?.exportsCreated || 0}</div>
              ...
              <IndustrialCard className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-white font-semibold">{userBehaviorMetrics?.featuresUsed.length || 0}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-300">Modules Accessed</span>
                    <span className="text-white font-semibold">{userBehaviorMetrics?.modulesUsed.length || 0}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-slate-300">Vectors Used</span>
                    <span className="text-white font-semibold">{userBehaviorMetrics?.vectorsUsed.length || 0}</span>
                  </div>
                </div>
              </IndustrialCard>
                  ...
                  {systemHealthMetrics && (
                    <>
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-white font-semibold">{formatDuration(systemHealthMetrics.uptime)}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-slate-300">Error Rate</span>
                        <div className="flex items-center gap-2">
                          {getHealthStatus(systemHealthMetrics.errorRate, { good: 1, warning: 5 }).icon}
                          <span
                            className={`font-semibold ${
                              getHealthStatus(systemHealthMetrics.errorRate, { good: 1, warning: 5 }).color
                            }`}
                          >
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="text-white">{Math.round(performanceMetrics.promptGenerationTime)}ms</span>
                        </div>
                        <IndustrialProgress
                          value={Math.min((performanceMetrics.promptGenerationTime / 2000) * 100, 100)}
                        />
                      </div>
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-slate-300">GPT Optimization</span>
                          <span className="text-white">{Math.round(performanceMetrics.gptOptimizationTime)}ms</span>
                        </div>
                        <IndustrialProgress
                          value={Math.min((performanceMetrics.gptOptimizationTime / 5000) * 100, 100)}
                        />
                      </div>
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          <span className="text-slate-300">Test Execution</span>
                          <span className="text-white">{Math.round(performanceMetrics.testExecutionTime)}ms</span>
                        </div>
                        <IndustrialProgress
