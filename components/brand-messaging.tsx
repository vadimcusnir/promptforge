...
export function BrandMessage({ messageId, className = "", showCTA = false, showSecondary = true }: BrandMessageProps) {
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      ...
      {showSecondary && message.secondary && <p className="text-sm text-slate-300">{message.secondary}</p>}
      {showCTA && message.cta && (
        <div className="pt-2">
          <IndustrialBadge variant="info" className="cursor-pointer hover:bg-blue-600 transition-colors">
            {message.cta}
          </IndustrialBadge>
        </div>
      )}
    ...
    <div className="text-center space-y-6 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-slate-400">Specialized AI modules engineered for specific business outcomes</p>
        </div>
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-slate-400">Multi-dimensional prompt optimization for maximum effectiveness</p>
        </div>
...
export function SocialProof() {
  const stats = [
    { value: "10,000+", label: "Active Users", icon: <Sparkles className="w-4 h-4" /> },
    { value: "99.9%", label: "Uptime SLA", icon: <Shield className="w-4 h-4" /> },
    { value: "10x", label: "Faster Development", icon: <TrendingUp className="w-4 h-4" /> },
    { value: "30 Days", label: "Average ROI", icon: <Zap className="w-4 h-4" /> },
  ]
    ...
    <div className="bg-gradient-to-r from-slate-800/50 to-slate-700/50 rounded-lg p-6">
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-slate-300">
          From Fortune 500 enterprises to innovative startups, teams choose PROMPTFORGE™ for mission-critical AI
          operations
        </p>
      </div>
  ...
  return (
    <div className="bg-gradient-to-r from-amber-500/20 to-orange-500/20 border border-amber-500/30 rounded-lg p-4">
      <div className="flex items-center justify-between">
        ...
        <IndustrialBadge variant="warning" className="cursor-pointer hover:bg-amber-600 transition-colors">
          {cta}
        </IndustrialBadge>
      </div>
    </div>
