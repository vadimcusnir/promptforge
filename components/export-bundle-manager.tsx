...
import { IndustrialCard, IndustrialButton, IndustrialBadge, IndustrialProgress } from "@/components/industrial-ui"
...
import { Package, Download, Settings, FileText, Database, BarChart3, Crown, Lock } from "lucide-react"
...
export function ExportBundleManager({ currentPrompt, editResults = [], testResults = [] }: ExportBundleManagerProps) {
  const [bundleName, setBundleName] = useState(`PROMPTFORGE Export ${new Date().toLocaleDateString()}`)
  ...
  const formatOptions: Array<{ value: ExportFormat; label: string; tier: string; description: string }> = [
    { value: "txt", label: "TXT Report", tier: "free", description: "Simple text report with basic statistics" },
    { value: "json", label: "JSON Data", tier: "free", description: "Structured data export for developers" },
    { value: "csv", label: "CSV Spreadsheet", tier: "pro", description: "Tabular data for analysis tools" },
    { value: "markdown", label: "Markdown", tier: "pro", description: "Documentation-ready format" },
    { value: "xml", label: "XML Data", tier: "pro", description: "Structured markup for integrations" },
    { value: "pdf", label: "PDF Report", tier: "pro", description: "Professional presentation format" },
    { value: "docx", label: "Word Document", tier: "enterprise", description: "Editable document format" },
    { value: "bundle", label: "Complete Bundle", tier: "enterprise", description: "All formats in one package" },
  ]
  ...
  const handleExport = async () => {
    try {
      ...
      if (!premiumGate.canExportFormat(selectedFormat)) {
        const requiredTier = formatOptions.find((f) => f.value === selectedFormat)?.tier || "pro"
        toast({
  ...
  const isFormatLocked = (tier: string) => {
    ...
    if (tier === "pro" && (currentTier.id === "pro" || currentTier.id === "enterprise")) return false
          ...
          <div className="space-y-6">
            <IndustrialCard className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-cyan-300 mb-2">Bundle Name</label>
                  <Input
                    value={bundleName}
                ...
                <div>
                  <label className="block text-sm font-medium text-cyan-300 mb-2">Export Format</label>
                  <Select value={selectedFormat} onValueChange={(value: ExportFormat) => setSelectedFormat(value)}>
                    ...
                    <SelectContent>
                      {formatOptions.map((option) => (
                        <SelectItem key={option.value} value={option.value} disabled={isFormatLocked(option.tier)}>
                          <div className="flex items-center gap-2">
                            {getFormatIcon(option.value)}
                            <span>{option.label}</span>
                            {isFormatLocked(option.tier) && <Lock className="w-3 h-3 text-amber-400" />}
                            <IndustrialBadge
                              variant={option.tier === "free" ? "default" : option.tier === "pro" ? "info" : "success"}
                              size="sm"
                            >
                              {option.tier}
                            </IndustrialBadge>
            ...
            <IndustrialCard className="p-6">
              ...
              <div className="space-y-3">
                {[
                  ...
                  { key: "edits", label: "GPT Optimizations", count: editResults.length },
            ...
            {(currentTier.id === "pro" || currentTier.id === "enterprise") && (
              <IndustrialCard className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-cyan-300 mb-2">Custom Header</label>
                    <Textarea
                      value={customization.customHeader}
                      onChange={(e) => setCustomization((prev) => ({ ...prev, customHeader: e.target.value }))}
                      className="industrial-input"
            ...
            <IndustrialCard className="p-6">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="text-center p-3 bg-slate-800/50 rounded-lg">
                    <div className="text-2xl font-bold text-green-400">{selectedFormat.toUpperCase()}</div>
                ...
                <div className="p-4 bg-slate-800/30 rounded-lg border border-slate-600/30">
                  ...
                  <div className="text-xs text-slate-400 mt-1">
                    Estimated size: ~{Math.max(50, Object.values(includeOptions).filter(Boolean).length * 25)} KB
                  </div>
                </div>
