...
export function ModuleCard({ module, isSelected, onSelect, onViewDetails, vectorColor }: ModuleCardProps) {
  ...
  return (
    ...
    >
          ...
          <div className="flex gap-1">
            {module.vectors.map((v) => (
              <Badge
                ...
                variant="secondary"
                className="text-xs"
                style={{ color: VECTORS[v as keyof typeof VECTORS]?.color || "#6b7280" }}
              >
                V{v}
              </Badge>
            ))}
      ...
      <h4 className="text-lg font-bold text-foreground mb-2 leading-tight break-words line-clamp-2">{module.name}</h4>
      ...
      <div className="mb-4 p-2 glass-strong rounded">
        ...
        <p className="text-sm font-bold text-green-400 break-words line-clamp-1 pl-5">{module.kpi}</p>
      </div>
