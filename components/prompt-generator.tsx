  ...
  return (
    <Card className="glass-effect p-6 glow-accent">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold font-[var(--font-heading)] text-foreground">Prompt Generator</h2>
        {validationResult && (
      ...
      <div className="mb-8 p-4 glass-strong rounded-lg">
        <div className="flex items-center justify-between text-sm">
          <div className={`flex items-center gap-2 ${selectedModule ? "text-green-400" : "text-muted-foreground"}`}>
            <div
          ...
          <div className={`flex items-center gap-2 ${generatedPrompt ? "text-green-400" : "text-muted-foreground"}`}>
            <div
      ...
      <Accordion type="multiple" defaultValue={["config", "prompt"]} className="space-y-4">
        {/* Config Summary Section */}
        <AccordionItem value="config">
          <AccordionTrigger className="text-lg font-semibold">
            <div className="flex items-center gap-2">
              <Settings className="w-5 h-5 text-primary" />📦 Configuration Summary
            </div>
          </AccordionTrigger>
          <AccordionContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              <div className="p-3 glass-strong rounded">
                ...
                <div className="font-semibold text-foreground">{config.domain}</div>
              </div>
              <div className="p-3 glass-strong rounded">
                <div className="text-xs text-muted-foreground mb-1">Scale</div>
                <div className="font-semibold text-foreground">{config.scale}</div>
              </div>
              <div className="p-3 glass-strong rounded">
                <div className="text-xs text-muted-foreground mb-1">Urgency</div>
                <div className="font-semibold text-foreground">{config.urgency}</div>
              </div>
              <div className="p-3 glass-strong rounded">
                <div className="text-xs text-muted-foreground mb-1">Complexity</div>
                <div className="font-semibold text-foreground">{config.complexity}</div>
              </div>
            </div>
            {generatedPrompt && (
              <div className="mt-4 p-3 glass-strong rounded text-xs text-muted-foreground">
                <strong>Session Hash:</strong> {generatedPrompt.hash} |<strong> Generated:</strong>{" "}
        ...
        {selectedModuleData && (
          <AccordionItem value="objective">
            <AccordionTrigger className="text-lg font-semibold">
              <div className="flex items-center gap-2">
                <Target className="w-5 h-5 text-blue-400" />🎯 Module Objective
              </div>
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-xl font-bold text-foreground">{selectedModuleData.name}</h3>
                </div>
                <p className="text-muted-foreground">{selectedModuleData.description}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div className="p-3 glass-strong rounded">
                    <div className="text-xs text-muted-foreground mb-1">📊 KPI Target</div>
                    <div className="font-semibold text-green-400">{selectedModuleData.kpi}</div>
                  </div>
                  <div className="p-3 glass-strong rounded">
                    <div className="text-xs text-muted-foreground mb-1">⚙️ Specification</div>
                    <div className="font-semibold text-foreground line-clamp-2">{selectedModuleData.spec}</div>
                  </div>
                </div>
              </div>
            </AccordionContent>
        ...
        {validationResult && (
          <AccordionItem value="metrics">
            <AccordionTrigger className="text-lg font-semibold">
              <div className="flex items-center gap-2">
                <BarChart3 className="w-5 h-5 text-green-400" />📈 Validation & Metrics
              </div>
            </AccordionTrigger>
            <AccordionContent>
              <div className="space-y-4">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="text-lg font-bold text-foreground">Overall Score: {validationResult.score}%</span>
                  <Badge variant={validationResult.score >= 80 ? "default" : "secondary"}>
                    {validationResult.score >= 80 ? "✅ Execution Ready" : "⚠️ Needs Review"}
                  </Badge>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
                  {[
                    { key: "hasTitle", label: "Title", value: validationResult.hasTitle },
                    { key: "hasContext", label: "Context", value: validationResult.hasContext },
                    { key: "hasKPI", label: "KPI", value: validationResult.hasKPI },
                    { key: "hasOutput", label: "Output", value: validationResult.hasOutput },
                    { key: "hasGuardrails", label: "Guardrails", value: validationResult.hasGuardrails },
                  ].map(({ key, label, value }) => (
                    <div key={key} className="p-3 glass-strong rounded text-center">
                      <div className={`text-2xl mb-1 ${value ? "text-green-400" : "text-red-400"}`}>
                        {value ? "✅" : "❌"}
                      </div>
        ...
        {generatedPrompt && (
          <AccordionItem value="telemetry">
            <AccordionTrigger className="text-lg font-semibold">
              <div className="flex items-center gap-2">
                <Activity className="w-5 h-5 text-orange-400" />📊 Session Telemetry
              </div>
            </AccordionTrigger>
            <AccordionContent>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div className="p-3 glass-strong rounded">
                  ...
                  <div className="text-lg font-bold text-green-400">{generatedPrompt.validationScore}%</div>
                </div>
                <div className="p-3 glass-strong rounded">
                  <div className="text-xs text-muted-foreground mb-1">KPI Compliance</div>
                  <div className="text-lg font-bold text-blue-400">{generatedPrompt.kpiCompliance}%</div>
                </div>
                <div className="p-3 glass-strong rounded">
                  <div className="text-xs text-muted-foreground mb-1">Structure Score</div>
                  <div className="text-lg font-bold text-purple-400">{generatedPrompt.structureScore}%</div>
                </div>
                <div className="p-3 glass-strong rounded">
                  <div className="text-xs text-muted-foreground mb-1">Clarity Score</div>
                  <div className="text-lg font-bold text-orange-400">{generatedPrompt.clarityScore}%</div>
                </div>
              </div>
            </AccordionContent>
          </AccordionItem>
