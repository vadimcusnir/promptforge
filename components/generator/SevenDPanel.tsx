...
const SEVEN_D_OPTIONS = {
  domain: ["business", "technical", "creative", "research", "marketing", "operations"],
...
const DOMAIN_DEFAULTS: Record<string, Partial<SevenDConfig>> = {
  ...
  operations: { scale: "enterprise", urgency: "critical", complexity: "expert" },
}

export function SevenDPanel({ selectedModule, onModuleChange, sevenDConfig, onConfigChange }: SevenDPanelProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="glass-effect rounded-lg p-6">
        <h2 className="text-xl font-semibold mb-4 font-montserrat">Module Selection</h2>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-lead-gray">{selectedModule.description}</div>
              </div>
            </div>
            <ChevronDown className={`w-5 h-5 transition-transform ${showModuleDropdown ? "rotate-180" : ""}`} />
          </button>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-lead-gray">{module.description}</div>
                    </div>
                  </div>
                </button>
              ))}
      ...
      <div className="glass-effect rounded-lg p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-semibold font-montserrat">7-D Configuration</h2>
        </div>

        <div className="grid gap-4">
          {Object.entries(SEVEN_D_OPTIONS).map(([key, options]) => (
            <div key={key}>
              <label className="block text-sm font-medium mb-2 capitalize">{key.replace("_", " ")}</label>
              <select
                value={sevenDConfig[key as keyof SevenDConfig]}
                onChange={(e) =>
                  key === "domain"
                    ? handleDomainChange(e.target.value)
                    : handleConfigChange(key as keyof SevenDConfig, e.target.value)
                }
