...
export function LayeredBackground() {
  ...
  useEffect(() => {
    ...
    const initLines = () => {
      ...
      const density = reducedMotion ? 0 : fpsTier === 'hi' ? 15 : fpsTier === 'mid' ? 12 : 8;
    ...
    const initShapes = () => {
      ...
      for (let i = 0; i < shapeCount; i++) {
        ...
        newShapes.push({
          id: `shape-${i}`,
          type: ['point', 'line', 'bar', 'circle'][Math.floor(Math.random() * 4)] as any,
          x,
          y,
          targetX: x,
          targetY: y,
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      lines.forEach(line => {
        const oscillationX = Math.sin(timestamp * 0.00005 + line.phase) * (line.oscillationX * 0.3);
        const oscillationY = Math.cos(timestamp * 0.00005 + line.phase) * (line.oscillationY * 0.3);
        const luminance = 1;

        ctx.globalAlpha = line.opacity * luminance;
        ctx.beginPath();
        ctx.moveTo(line.x + oscillationX, line.y + oscillationY);
        ctx.lineTo(line.x + line.width + oscillationX, line.y + line.height + oscillationY);
        ctx.stroke();
      });
      ...
      words.forEach(word => {
        const oscillationX = Math.sin(timestamp * 0.0001 + word.phase) * (word.oscillationX * 0.5);
        const oscillationY = Math.cos(timestamp * 0.0001 + word.phase) * (word.oscillationY * 0.5);
        const scale = 1 + Math.sin(timestamp * 0.0002 + word.phase) * 0.01;

        let baseOpacity = word.opacity + Math.sin(timestamp * 0.0003 + word.phase) * 0.1;
        if (quoteActive) {
      ...
      shapes.forEach(shape => {
        ...
        shape.targetX += Math.sin(timestamp * moveSpeed + shape.phase) * curveIntensity;
        shape.targetY += Math.cos(timestamp * moveSpeed + shape.phase * 1.3) * curveIntensity;
      ...
      if (timestamp - lastQuoteTime > quoteInterval && quotes.length < maxQuotes) {
  ...
  if (reducedMotion) {
    return <div className="fixed inset-0 bg-black opacity-10 pointer-events-none z-0" />;
  }
