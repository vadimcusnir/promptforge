  ...
  const initializeTokens = useCallback(() => {
    ...
    return Array.from({ length: tokenCount }, (_, index) => {
      ...
      const token: ActiveToken = {
        ...
        opacity: Math.random() * (template.opacityRange[1] - template.opacityRange[0]) + template.opacityRange[0],
  ...
  const animate = useCallback((currentTime: number) => {
    ...
    setTokens(prevTokens => 
      prevTokens.map(token => {
        ...
        const newX = token.x + Math.sin(driftCycle + token.animationDelay) * token.jitter * driftMultiplier
        const newY = token.y + Math.cos(driftCycle + token.animationDelay) * token.jitter * driftMultiplier

        // Opacity oscillation
        const opacityCycle = (currentTime / (8000 + Math.random() * 4000)) % (Math.PI * 2)
        const baseOpacity = Math.sin(opacityCycle) * 0.3 + 0.7
        const newOpacity = Math.max(0.1, Math.min(1.0, 
