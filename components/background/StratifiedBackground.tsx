...
const AI_TOKENS = [
  ...
  "softmax", "relu", "lstm", "gru", "conv", "pool", "matrix", "tensor",
  "optimizer", "learning_rate", "dropout", "activation", "loss", "accuracy"
]
...
export function StratifiedBackground({ className = "" }: StratifiedBackgroundProps) {
  ...
  useEffect(() => {
    ...
    const shapeArray = Array.from({ length: 15 }, (_, i) => ({
      id: `shape-${i}`,
      symbol: GEOMETRIC_SHAPES[Math.floor(Math.random() * GEOMETRIC_SHAPES.length)],
      x: Math.random() * 100,
      y: Math.random() * 100,
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTokens(prev => prev.map(token => ({
        ...
        y: (token.y + Math.sin(Date.now() / 10000 + token.id.charCodeAt(5)) * 0.1 + 100) % 100,
        opacity: Math.max(0.1, Math.min(0.5, token.opacity + (Math.random() - 0.5) * 0.02))
      })))
    }, 150)
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setShapes(prev => prev.map(shape => ({
        ...
        scale: 0.5 + Math.sin(Date.now() / 8000 + shape.id.charCodeAt(6)) * 0.3 + 0.5
      })))
    }, 200)
