...
export function DigitalRune() {
  ...
  useEffect(() => {
    if (prefersReducedMotion) {
      ...
      setSymbols([
        { symbol: ">", x: 50, y: 50, opacity: 0.8, scale: 2, rotation: 0, glitch: false },
        { symbol: "|", x: 45, y: 50, opacity: 0.4, scale: 1.5, rotation: 0, glitch: false },
        { symbol: "{", x: 55, y: 50, opacity: 0.4, scale: 1.5, rotation: 0, glitch: false },
        { symbol: "~", x: 50, y: 45, opacity: 0.3, scale: 1.2, rotation: 0, glitch: false },
      ])
      return
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setSymbols(prev => prev.map(symbol => ({
        ...
        symbol: symbol.glitch && Math.random() < 0.3 
