...
type TokenView = { id: number; text: string; x: number; y: number; delay: number }
...
export default function MatrixTokens() {
  ...
  useEffect(() => {
    ...
    const updateReduced = () => {
      const isReduced = mediaQuery.matches && document.documentElement.getAttribute("data-animations") !== "on"
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      console.log("[v0] Token density check - expected:", actualTokens, "actual:", tokenElements.length)
