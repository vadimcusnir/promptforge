...
export default function MatrixTokens() {
  const pool = useMemo(
    () => createTokenPool(matrixTokens, backgroundConfig.maxTokens),
    [],
  );
  ...
  useEffect(() => {
    ...
    const updateReduced = () => {
      const isReduced =
        mediaQuery.matches &&
        document.documentElement.getAttribute("data-animations") !== "on";
  ...
  useEffect(() => {
    ...
    if (
      document.documentElement.classList.contains("matrix-animations-ready")
    ) {
      setReady(true);
