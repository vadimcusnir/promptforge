...
export async function uploadFile(
  ...
  const { data, error } = await supabase.storage.from('bundles').upload(path, content, {
    contentType,
  ...
  const { data: urlData } = await supabase.storage.from('bundles').createSignedUrl(path, 3600);
...
export async function uploadArtifacts(
  ...
  for (const artifact of artifacts) {
    ...
    try {
      const result = await uploadFile(fullPath, artifact.content, artifact.contentType);
