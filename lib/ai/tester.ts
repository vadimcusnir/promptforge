...
export async function testPromptMultipleScenarios(
  ...
  for (const scenario of scenarios) {
    const result = await testPromptLive(prompt, scenario.input, scenario.expectedOutcome, options);
    results.push(result);
  }
...
function evaluateSuccess(output: string, expectedOutcome: string): boolean {
  ...
  const expectedKeywords = expectedLower.split(/\s+/).filter(word => word.length > 3); // Filter out short words

  const presentKeywords = expectedKeywords.filter(keyword => outputLower.includes(keyword));
