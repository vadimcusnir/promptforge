...
function generateTestOutput(
  ...
  const baseResponse =
    contextualResponses[
      prompt.config.domain as keyof typeof contextualResponses
    ] || "standard implementation";
  const urgencyResponse =
    urgencyAdaptations[
      prompt.config.urgency as keyof typeof urgencyAdaptations
    ] || "standard implementation";
...
function calculateTestScores(
  ...
  const overall = Math.round(
    (structure + kpiCompliance + clarity + executability) / 4,
  );
...
function validatePromptStructure(
  ...
  const hasRequiredSections =
    prompt.includes("## CONTEXT") && prompt.includes("## OBJECTIVE");
  const hasValidKPIs =
    prompt.includes("## KPI") && prompt.includes(module?.kpi || "");
  const hasGuardrails = prompt.includes("## GUARDRAILS");
  const hasFailsafes =
    prompt.includes("FAIL-SAFES") || prompt.includes("Alternative");
  const isExecutable =
    prompt.includes("## NEXT ACTIONS") || prompt.includes("SPECIFICATIONS");
...
function generateRecommendations(
  ...
  if (scores.overall < 85) {
    recommendations.push(
      "Consider optimization with GPT Editor for improvements",
    );
  }
...
export function compareTestResults(results: TestResult[]): {
  ...
  if (results.length > 1) {
    ...
    const kpiChange =
      latest.scores.kpiCompliance - previous.scores.kpiCompliance;
    const clarityChange = latest.scores.clarity - previous.scores.clarity;
    const executabilityChange =
      latest.scores.executability - previous.scores.executability;

    trends.push(
      {
        metric: "Structure",
        trend:
          structureChange > 2 ? "up" : structureChange < -2 ? "down" : "stable",
      ...
      },
      {
        metric: "Clarity",
        trend:
          clarityChange > 2 ? "up" : clarityChange < -2 ? "down" : "stable",
        change: clarityChange,
      },
      {
        metric: "Executability",
        trend:
          executabilityChange > 2
            ? "up"
            : executabilityChange < -2
              ? "down"
              : "stable",
    ...
    if (structureChange > 5)
      improvements.push("Prompt structure improved significantly");
    if (kpiChange > 5)
      improvements.push("KPI compliance increased considerably");
    if (clarityChange > 5) improvements.push("Instruction clarity improved");
    if (executabilityChange > 5)
      improvements.push("Prompt executability increased");
  }
