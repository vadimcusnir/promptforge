...
export async function runPromptTest(
  ...
  const executionDelay =
    options.mode === 'quick' ? 1000 : options.mode === 'comprehensive' ? 2500 : 4000;
...
function generateTestOutput(prompt: GeneratedPrompt, module: any, options: TestOptions): string {
  ...
  const baseResponse =
    contextualResponses[prompt.config.domain as keyof typeof contextualResponses] ||
    'standard implementation';
  const urgencyResponse =
    urgencyAdaptations[prompt.config.urgency as keyof typeof urgencyAdaptations] ||
    'standard implementation';
