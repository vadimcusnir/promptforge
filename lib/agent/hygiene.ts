...
export function stripSecrets(input: string): string {
  ...
  // JWT tokens
  sanitized = sanitized.replace(/eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/g, 'eyJ***REDACTED***');
...
export function sanitizeMarkdown(markdown: string): string {
  ...
  let sanitized = DOMPurify.sanitize(markdown, {
    ALLOWED_TAGS: ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'br', 'strong', 'em', 'ul', 'ol', 'li', 'blockquote', 'code', 'pre', 'a'],
