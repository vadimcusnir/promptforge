...
export class ExportBundleEngine {
  ...
  async createBundle(
    ...
    if (!this.premiumGate.canExportFormat(format)) {
      throw new Error(`Format ${format} requires ${format === "pdf" || format === "docx" ? "Pro" : "Enterprise"} plan`)
    }
    ...
    const content: BundleContent = {
      ...
      history: options.includeHistory !== false ? history.slice(0, currentTier.limits.historyRetention) : [],
    ...
    return {
      ...
      qualityMetrics: {
        ...
        averageStructureScore:
          ...
          Math.max(promptEntries.length, 1),
        averageClarityScore:
          promptEntries.reduce((sum, e) => sum + (e.metadata.clarityScore || 0), 0) / Math.max(promptEntries.length, 1),
  ...
  private calculateImprovementTrends(entries: any[]): Array<{ date: string; score: number }> {
    ...
    for (let i = 0; i < sortedEntries.length; i += Math.max(1, Math.floor(sortedEntries.length / 10))) {
  ...
  private async exportAsTXT(bundle: ExportBundle): Promise<Blob> {
    ...
    if (bundle.content.history.length > 0) {
      ...
      bundle.content.history.slice(0, 10).forEach((entry, index) => {
        lines.push(`${index + 1}. [${entry.type.toUpperCase()}] ${entry.moduleName} (V${entry.vector})`)
...
<promptforge-bundle>
  ...
  private async exportAsBundle(bundle: ExportBundle): Promise<Blob> {
    ...
    return new Blob([JSON.stringify(bundleData, null, 2)], { type: "application/json" })
  }
