  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (trimmed.includes(":") && !trimmed.startsWith(" ")) {
        ...
        if (value && value.trim()) {
          config[key.trim()] = value.trim() === "true" ? true : value.trim() === "false" ? false : value.trim()
        } else {
          currentKey = key.trim()
          config[currentKey] = {}
        ...
        const [key, value] = trimmed.split(":", 2)
        currentSection[key.trim()] = value.trim() === "true" ? true : value.trim() === "false" ? false : value.trim()
      }
    }
...
export class AgentWatchWorker {
  ...
  private analyzeMetrics(): void {
    ...
    const avgScore = recentMetrics
      .filter(m => m.score !== undefined)
      .reduce((sum, m) => sum + (m.score || 0), 0) / recentMetrics.filter(m => m.score !== undefined).length
    ...
    const lowScoreMetrics = recentMetrics.filter(m => m.score !== undefined && m.score < this.thresholds.minScore)
    if (lowScoreMetrics.length > recentMetrics.length * 0.3) { // More than 30% of runs have low scores
      this.triggerAlert({
        type: "score_low",
        severity: "warning",
  ...
  private handleCriticalAlert(alert: AnomalyAlert): void {
    switch (alert.type) {
      ...
      case "error_spike":
        ...
        if (alert.actual > 0.2) { // 20% error rate
  ...
  getMetricsSummary(): {
    ...
    return {
      ...
      avgTokens: recentMetrics.reduce((sum, m) => sum + m.tokens, 0) / Math.max(recentMetrics.length, 1),
      avgCost: recentMetrics.reduce((sum, m) => sum + m.cost, 0) / Math.max(recentMetrics.length, 1),
      avgScore: recentMetrics
        .filter(m => m.score !== undefined)
        .reduce((sum, m) => sum + (m.score || 0), 0) / Math.max(recentMetrics.filter(m => m.score !== undefined).length, 1),
      errorRate: recentMetrics.reduce((sum, m) => sum + m.errorRate, 0) / Math.max(recentMetrics.length, 1),
      degradationMode: this.degradationMode,
      recentAlerts: recentMetrics.filter(m => 
