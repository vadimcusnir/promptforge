...
test.describe('Glitch Protocol v1 Compliance', () => {
  ...
  test('should respect max 6 keywords per page limit', async ({ page }) => {
  ...
  test('should have correct contract markup structure', async ({ page }) => {
  ...
  test('should have accessible overlay elements', async ({ page }) => {
  ...
  test('should respect reduced motion preferences', async ({ page }) => {
    ...
    const glitchScript = await page.evaluate(() => {
      return document.querySelector('script[src="/glitch-keywords.js"]') !== null;
    });
  ...
  test('should respect data-motion="off" attribute', async ({ page }) => {
  ...
  test('should enforce performance constraints', async ({ page }) => {
  ...
  test('should prevent infinite loops', async ({ page }) => {
  ...
  test('should have anti-CLS measures in place', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const styles = await firstOverlay.evaluate(el => {
  ...
  test('should have proper CSS containment for performance', async ({ page }) => {
    const containers = page.locator('.kw[data-glitch]');
    const count = await containers.count();
    
    if (count > 0) {
      const firstContainer = containers.first();
      const containProperty = await firstContainer.evaluate(el => {
  ...
  test('should validate H1/H2 targeting only', async ({ page }) => {
...
test.describe('Glitch Protocol Telemetry', () => {
  test('should expose telemetry data', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
