...
test.describe('Header & Footer', () => {
  ...
  test('header maintains stable height on hover and scroll', async ({ page }) => {
  ...
  test('mobile navigation has proper ARIA and focus management', async ({ page }) => {
    // Set mobile viewport
  ...
  test('navigation has proper aria-current for active page', async ({ page }) => {
  ...
  test('footer links are accessible and functional', async ({ page }) => {
    ...
    const firstProductLink = productSection.locator('..').locator('ul a').first()
  ...
  test('telemetry events are emitted on interactions', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      window.addEventListener('cta_primary_click', (e: any) => {
        (window as any).logEvent({ type: 'cta_primary_click', detail: e.detail })
      })
