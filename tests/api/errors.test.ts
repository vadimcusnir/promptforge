...
describe('Standardized Error System', () => {
  describe('API_ERROR_CODES', () => {
    it('should have correct error codes and status mappings', () => {
      ...
      expect(API_ERROR_CODES.ENTITLEMENT_REQUIRED.type).toBe('ENTITLEMENT_REQUIRED');
      ...
      expect(API_ERROR_CODES.INPUT_SCHEMA_MISMATCH.type).toBe('INPUT_SCHEMA_MISMATCH');
      ...
      expect(API_ERROR_CODES.INTERNAL_RUN_ERROR.type).toBe('INTERNAL_RUN_ERROR');
    });
  ...
  describe('StandardAPIError', () => {
    it('should create error with correct properties', () => {
      const error = new StandardAPIError('INVALID_7D_ENUM', { field: 'domain' });
    ...
    it('should accept custom message', () => {
      ...
      const error = new StandardAPIError('UNAUTHENTICATED', null, customMessage);
  ...
  describe('createErrorResponse', () => {
    it('should create proper error response', async () => {
      const response = createErrorResponse('INVALID_7D_ENUM', { field: 'domain' });
      ...
      expect(response.headers.get('Access-Control-Allow-Origin')).toBe('https://chatgpt-prompting.com');
    });

    it('should handle custom message', async () => {
      const customMessage = 'Custom validation error';
      const response = createErrorResponse('INPUT_SCHEMA_MISMATCH', null, customMessage);
  ...
  describe('createEntitlementErrorResponse', () => {
    it('should create entitlement error with single requirement', async () => {
      const response = createEntitlementErrorResponse('canUseGptTestReal', 'free');
    ...
    it('should create entitlement error with multiple requirements', async () => {
      const response = createEntitlementErrorResponse(['hasAPI', 'plan_enterprise']);

      const body = await response.json();
      expect(body.details.required_entitlements).toEqual(['hasAPI', 'plan_enterprise']);
    });
  });

  describe('create7DErrorResponse', () => {
    it('should create 7D validation error', async () => {
      const response = create7DErrorResponse('domain', 'invalid_value', ['fintech', 'saas']);
  ...
  describe('createCORSResponse', () => {
    it('should create proper CORS preflight response', () => {
      ...
      expect(response.headers.get('Access-Control-Allow-Headers')).toContain('x-pf-key');
  ...
  describe('Error Code Consistency', () => {
    it('should map all specified error codes correctly', () => {
      ...
      specifiedErrors.forEach(({ code, status }) => {
        const errorConfig = API_ERROR_CODES[code as keyof typeof API_ERROR_CODES];
    ...
    it('should have consistent error type naming', () => {
      Object.entries(API_ERROR_CODES).forEach(([key, config]) => {
        ...
        const validTypes = [key, 'INVALID_7D_ENUM', 'UNAUTHENTICATED', 'ENTITLEMENT_REQUIRED', 'MODULE_NOT_FOUND', 'RULESET_CONFLICT', 'INPUT_SCHEMA_MISMATCH', 'RATE_LIMITED', 'INTERNAL_RUN_ERROR'];
