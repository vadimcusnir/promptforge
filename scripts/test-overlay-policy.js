...
function checkLayoutIntegration() {
  ...
  if (missingIntegrations.length > 0) {
    errors.push(`❌ Missing layout integrations: ${missingIntegrations.join(', ')}`);
    return false;
  }
...
function checkE2ETests() {
  const testPath = path.join(__dirname, '../tests/e2e/ui-overlay-policy.spec.ts');
...
function checkDocumentation() {
  const docsPath = path.join(__dirname, '../docs/ui/overlay-policy-operations.txt');
  ...
  console.log('✅ Operations Documentation (docs/ui/overlay-policy-operations.txt)');
  return true;
}
...
function checkGuardrails() {
  ...
  if (!fs.existsSync(codeownersPath)) {
    warnings.push('⚠️  CODEOWNERS not found - consider adding for mandatory reviews');
    guardrailsOk = false;
  } else {
