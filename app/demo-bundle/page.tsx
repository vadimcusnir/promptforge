...
const files = [
  { name: "prompt.txt", path: `${BASE}/prompt.txt`, desc: "Raw prompt instruction" },
  { name: "prompt.md", path: `${BASE}/prompt.md`, desc: "Readable spec + KPI" },
  { name: "prompt.json", path: `${BASE}/prompt.json`, desc: "Config + metadata" },
  { name: "prompt.pdf", path: `${BASE}/prompt.pdf`, desc: "Branded PDF export" },
  { name: "telemetry.json", path: `${BASE}/telemetry.json`, desc: "Scores, tokens, timings" },
  { name: "manifest.json", path: `${BASE}/manifest.json`, desc: "Bundle manifest" },
  { name: "checksum.txt", path: `${BASE}/checksum.txt`, desc: "SHA256 canonical checksum" },
];
    ...
    <section className="min-h-screen px-6 py-16 max-w-4xl mx-auto text-white bg-[#0a0a0a]">
      <div className="mb-8">
        ...
        <p className="text-lg opacity-80 leading-relaxed">
          Explore a real PromptForge export bundle. Download individual artifacts, or grab everything as a ZIP.
        </p>
        <div className="mt-4 flex flex-wrap gap-2 text-sm">
          <span className="px-3 py-1 bg-[#d1a954]/20 text-[#d1a954] rounded-full">Module M12</span>
          <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-full">Quality: 9.1/10</span>
      ...
      <div className="grid gap-4 mb-8">
        {files.map((f) => (
          <div key={f.name} className="rounded-lg border border-white/15 bg-white/5 backdrop-blur-sm p-6 hover:bg-white/10 transition-colors">
      ...
      <div className="bg-white/5 rounded-lg border border-white/15 p-6">
        ...
        <div className="mt-4 pt-4 border-t border-white/15">
          <p className="text-sm text-white/70">
            Bundle format & checksum follow the <span className="text-[#d1a954]">PROMPTFORGE export standard</span>.{" "}
            <span className="text-white/90">Artifacts: .txt, .md, .json, .pdf, manifest, checksum.</span>
          </p>
          <p className="text-xs text-white/50 mt-2">
            © PROMPTFORGE v3 — Demo License
          </p>
        </div>
      </div>
    </section>
