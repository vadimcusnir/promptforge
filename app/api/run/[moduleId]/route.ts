  ...
  try {
    ...
    if (validatedInput.api_key) {
      ...
      if (!apiKey.scopes?.includes('run_modules')) {
        return NextResponse.json({ error: 'API key lacks run_modules scope' }, { status: 403 });
      }
    ...
    if (orgId) {
      await withEntitlementGate(orgId, ['canUseAllModules'], async () => {
        ...
        const { data: allowedModules } = await supabase.rpc('pf_get_allowed_modules', {
          p_org: orgId,
        });
    ...
    if (validatedInput.api_key || (orgId && (await hasEntitlement(orgId, 'hasEvaluatorAI')))) {
      evaluation = await evaluatePrompt(
        ...
        normalizedParams?.domain || 'general'
      );
...
// Helper functions
async function generateRunHash(
  params: {
    domain: string;
    scale: string;
    urgency: string;
