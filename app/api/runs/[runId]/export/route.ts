...
export async function GET(
  ...
  try {
    ...
    if (format === 'pdf' && !entitlements.features.canExportPDF) {
      return NextResponse.json(
        { 
    ...
    if (format === 'json' && !entitlements.features.canExportJSON) {
      return NextResponse.json(
        { 
          error: 'ENTITLEMENT_REQUIRED',
    ...
    switch (format) {
      case 'json':
        ...
        filename = `${run.modules?.[0]?.name || 'run'}_${runId}.json`
        break

      case 'txt':
        content = generateTextExport(exportData)
        contentType = 'text/plain'
        filename = `${run.modules?.[0]?.name || 'run'}_${runId}.txt`
        break

      case 'md':
        content = generateMarkdownExport(exportData)
        contentType = 'text/markdown'
        filename = `${run.modules?.[0]?.name || 'run'}_${runId}.md`
        break
