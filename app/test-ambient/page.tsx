      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
          <div className="bg-[#1a1a1a] p-6 rounded-lg border border-[#5a5a5a]/30">
            <h2 className="text-xl font-bold mb-4 text-[#d1a954]">Current Status</h2>
            <div className="space-y-2">
              <p><strong>Motion Mode:</strong> <span className={`px-2 py-1 rounded text-sm ${
                motionMode === 'off' ? 'bg-green-900 text-green-200' : 
                motionMode === 'on' ? 'bg-blue-900 text-blue-200' : 
                'bg-gray-900 text-gray-200'
              }`}>{motionMode}</span></p>
              <p><strong>Background:</strong> {motionMode === 'off' ? 'Static Grid Only' : 'Normal Mode'}</p>
              <p><strong>Animations:</strong> {motionMode === 'off' ? 'Disabled (CSS)' : 'Enabled'}</p>
            </div>
          </div>
        ...
        <div className="bg-[#1a1a1a] p-6 rounded-lg border border-[#5a5a5a]/30 mb-8">
          <h2 className="text-xl font-bold mb-4 text-[#d1a954]">What You Should See</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="font-bold text-green-400 mb-2">🔇 Ambient Mode (OFF)</h3>
            ...
            <div>
              <h3 className="font-bold text-blue-400 mb-2">🌊 Normal Mode (ON)</h3>
              <ul className="text-sm space-y-1 text-gray-300">
        ...
        <div className="bg-[#1a1a1a] p-6 rounded-lg border border-[#5a5a5a]/30">
          <h2 className="text-xl font-bold mb-4 text-[#d1a954]">Technical Details</h2>
          <div className="text-sm space-y-2 font-mono">
            <p><strong>CSS Rule:</strong> <code>html[data-motion="off"] * {`{`} animation: none !important; transition: none !important; {`}`}</code></p>
            <p><strong>Background:</strong> <code>&lt;BackgroundRoot ambient /&gt;</code></p>
            <p><strong>API:</strong> <code>POST /api/motion</code> with <code>{`{"on": false}`}</code></p>
            <p><strong>Cookie:</strong> <code>motion=off|on</code></p>
          </div>
        </div>
      </div>
    </div>
